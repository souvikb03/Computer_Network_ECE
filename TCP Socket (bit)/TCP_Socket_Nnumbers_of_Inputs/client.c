...
int main() {
    ...
    printf("Enter the data stream (%d integers):\n", n);
    for (i = 0; i < n; i++) {
        scanf("%d", &stream[i]);
    }
    ...
    if (write(sockfd, &n, sizeof(int)) == -1) {
    ...
    int modified_size;
    if (read(sockfd, &modified_size, sizeof(int)) == -1) {
        perror("Error reading from socket");
        close(sockfd);
        exit(1);
    }

    stream = (int *)realloc(stream, modified_size * sizeof(int));
